...
public class Jedis extends BinaryJedis implements JedisCommands, MultiKeyCommands,
    ...
    private Set<GeoPositionTuple> getGeoPositionTupledSet() {
        checkIsInMultiOrPipeline();
        List<Object> membersWithScores = client.getObjectMultiBulkReply();
        if (membersWithScores == null) {
            return null;
        }
        if (membersWithScores.size() == 0) {
        ...
        Set<GeoPositionTuple> set = new LinkedHashSet<GeoPositionTuple>(membersWithScores.size() / 2, 1.0f);
        for (Object membersWithScore : membersWithScores) {
            ArrayList currentElement = (ArrayList) membersWithScore;
            String member = new String((byte[])currentElement.get(0));
            ArrayList longlat = (ArrayList)(currentElement.get(1));
            Double longitude = Double.valueOf(new String((byte[])longlat.get(0)));
            Double latitude = Double.valueOf(new String((byte[])longlat.get(1)));

            set.add(new GeoPositionTuple(member, latitude, longitude));
        }
        return set;
    }
    ...
    @Override
    public Set<GeoPositionTuple> geoRadiusByMemberWithCoord(String key, String member, double distance, String unit) {
        checkIsInMultiOrPipeline();
        client.geoRadiusByMemberWithCoord(key, member, distance, unit);
        return getGeoPositionTupledSet();
    }
